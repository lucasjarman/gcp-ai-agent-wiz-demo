#!/bin/bash
# Wiz Pre-Commit Security Gate
# PreToolUse hook for Bash — intercepts `git commit` commands and runs wizcli dir scan.
#
# Behavior:
#   WIZ_HOOK_MODE=audit  (default) → warn via permissionDecisionReason, allow commit
#   WIZ_HOOK_MODE=block            → deny via permissionDecision, block commit
#
...
# Only intercept git commit commands (including those chained after &&, ;, ||, or |)
if [[ ! "$cmd" =~ (^|[[:space:]]|;|&|\|)git[[:space:]]+commit ]]; then
  exit 0
fi

# --- Split chained git add && git commit into separate commands ---
# PreToolUse fires before the entire command, so git add hasn't staged files yet.
# Use updatedInput to strip the commit, let git add run, then Claude retries git commit separately.
if echo "$cmd" | grep -q 'git add' && echo "$cmd" | grep -q 'git commit'; then
  # Collapse newlines — Claude sends multi-line commit messages (heredoc or quoted)
  add_only=$(printf '%s' "$cmd" | tr '\n' ' ' | sed 's/[&|;]*[[:space:]]*git[[:space:]]*commit.*//')
  jq -n --arg cmd "$add_only" '{
    "hookSpecificOutput": {
      "hookEventName": "PreToolUse",
      "permissionDecision": "allow",
      "updatedInput": { "command": $cmd },
      "additionalContext": "The git commit was removed from this command. After git add completes, run git commit as a separate Bash command so the Wiz security scan can analyze staged files."
    }
  }'
  exit 0
fi
...
INNER_TIMEOUT="${WIZ_SCAN_TIMEOUT:-45}"

export WIZ_AGENT="claude_code"
export WIZ_AGENT_TRIGGER="pre-commit"
...
# --- Check wizcli meets minimum version (v1.42 required) ---
WIZ_MIN_VERSION="1.42"
WIZCLI_VERSION=$("$WIZCLI" version 2>&1 | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | head -1 | sed 's/^v//')
WIZCLI_MAJOR=$(echo "$WIZCLI_VERSION" | cut -d. -f1)
WIZCLI_MINOR=$(echo "$WIZCLI_VERSION" | cut -d. -f2)
if [ -z "$WIZCLI_MINOR" ] || [ "$WIZCLI_MAJOR" -lt 1 ] || { [ "$WIZCLI_MAJOR" -eq 1 ] && [ "$WIZCLI_MINOR" -lt 42 ]; }; then
  jq -n --arg ver "${WIZCLI_VERSION:-unknown}" --arg min "$WIZ_MIN_VERSION" '{
    "systemMessage": ("⚠️ Wiz CLI v" + $ver + " is outdated (minimum required: v" + $min + "). Run /wizcli-setup to update to the latest version."),
    "hookSpecificOutput": {
      "hookEventName": "PreToolUse",
      "permissionDecision": "allow",
      "permissionDecisionReason": ("Wiz CLI v" + $ver + " is below minimum required v" + $min + ". Skipping security scan."),
      "additionalContext": "IMPORTANT: You MUST inform the user: Wiz CLI must be updated to v" + $min + " or later. Run /wizcli-setup to install the latest version. The pre-commit security scan will not run until the CLI is updated."
    }
  }'
  exit 0
fi
...
# Scans only staged files (git add must have run before this point — see chained command split above)
# Use background process + kill timer
printf "🔍 Running Wiz security scan (pre-commit)...\n" > /dev/tty 2>/dev/null || true
scan_output_file=$(mktemp)
scan_stderr_file=$(mktemp)
"$WIZCLI" scan dir . --git-staged-only --stdout json --use-device-code --agent-version="$WIZ_AGENT_VERSION" --agent-operation="$WIZ_AGENT_TRIGGER" --by-policy-hits="BLOCK" --by-policy-hits="AUDIT" --no-publish > "$scan_output_file" 2>"$scan_stderr_file" &
scan_pid=$!
...
if [ $wizcli_exit -eq 0 ]; then
  ...
  if [ "$verdict" = "WARN_BY_POLICY" ]; then
    ...
    jq -n '{
      "systemMessage": "✅ Wiz security scan (pre-commit) passed. No issues found.",
      "hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "permissionDecision": "allow",
        "permissionDecisionReason": "Wiz security scan passed. No issues found.",
        "additionalContext": "IMPORTANT: You MUST display the following to the user as a markdown blockquote before continuing:\n\n> **Wiz Security Scan (pre-commit):** Passed. No issues found.\n\nDo NOT skip this."
      }
    }'
    exit 0
  fi
...
summary=$(echo "$scan_output" | jq -r '
  .result.analytics as $a |
  {
    ...
    "secrets": "Secrets",
    "malware": "Malware",
    "softwareSupplyChain": "Software Supply Chain",
    "sast": "SAST"
  } as $names |
  ...
  "Wiz security scan found: " + ($tc|tostring) + " critical, " + ($th|tostring) + " high, " + ($tm|tostring) + " medium, " + ($tl|tostring) + " low, " + ($ti|tostring) + " info issue(s)." +
...
if [ -z "$summary" ]; then
  summary="Wiz security scan found issues but could not parse counts."
fi

# --- Build structured findings from all scanners ---
findings=$(echo "$scan_output" | jq -r '
  ([.result.libraries // [] | .[] |
    select(.vulnerabilities | length > 0) |
    .name as $pkg | .version as $ver | .path as $path | .startLine as $line |
    (.vulnerabilities |
      (map(select(.hasExploit == true)) | length) as $exploitable |
      (map(select(.hasCisaKevExploit == true)) | length) as $kev |
      (map(select(.fixedVersion == null)) | length) as $unfixable |
      map({severity, fixedVersion}) |
      group_by(.severity) |
      map({
        severity: .[0].severity,
        count: length,
        best_fix: (map(.fixedVersion // empty) | if length > 0 then (sort | last) else null end)
      }) |
      {
        by_sev: .,
    ...
    ) as $info |
    {
      ...
      findings: ($info.by_sev | map(.severity + ":" + (.count|tostring)) | join(", ")),
      fix: ($info.by_sev | map(.best_fix // empty) | if length > 0 then (sort | last) else "no fix available" end),
      exploitable: $info.exploitable,
      kev: $info.kev,
      unfixable: $info.unfixable
    }
  ]) as $vulns |
  ...
  ([.result.softwareSupplyChain // [] | .[] | {
    name: (.name // "unknown"),
    severity: (.severity // "HIGH"),
    licenses: ([.licenseNames // [] | .[]] | join(", ")),
    package: ((.codeLibrary.name // "unknown") + "==" + (.codeLibrary.version // "?")),
    path: (.codeLibrary.path // "unknown")
  }]) as $ssc |

  (if ($vulns | length) > 0 then
    "VULNERABILITIES (" + ($vulns | length | tostring) + " packages):\n" +
    ($vulns | map(
      "  - " + .package + " (" + .path + ") — " + .findings + " → upgrade to " + .fix +
      (if .exploitable > 0 then " [EXPLOITABLE:" + (.exploitable|tostring) + "]" else "" end) +
      (if .kev > 0 then " [CISA-KEV]" else "" end) +
      (if .unfixable > 0 then " [" + (.unfixable|tostring) + " unfixable]" else "" end)
    ) | join("\n")) + "\n\n"
  else "" end) +
  ...
  (if ($sast | length) > 0 then
    "SAST (" + ($sast | length | tostring) + " findings):\n" +
    ($sast | map("  - " + .rule + " at " + .path + ":" + (.line|tostring) + " [" + .severity + "] " + .description[:100]) | join("\n")) + "\n\n"
  else "" end) +
  ...
  (if ($ssc | length) > 0 then
    "SOFTWARE SUPPLY CHAIN (" + ($ssc | length | tostring) + " findings):\n" +
    ($ssc | map("  - " + .package + " (" + .path + ") — " + .name + " [" + .severity + "] licenses: " + .licenses) | join("\n")) + "\n\n"
  else "" end)
' 2>/dev/null || echo "")
...
jq -n --arg msg "$summary" --arg findings "$findings" '{
  ...
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": ($msg + " Remediate findings before committing."),
    "additionalContext": (
      if ($findings | length) > 0 then
        "The commit was BLOCKED due to security policy violations.\n\n" +
